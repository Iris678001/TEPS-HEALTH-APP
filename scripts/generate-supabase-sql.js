...
async function exportDatabase() {
  console.log('Reading data from local database with verified schema mappings...');
  const students = await prisma.student.findMany({ orderBy: { admissionNumber: 'asc' } });
  const checkups = await prisma.healthCheckup.findMany({ orderBy: { id: 'asc' } });
  const observations = await prisma.observation.findMany({ orderBy: { id: 'asc' } });
  const immunizations = await prisma.immunization.findMany({ orderBy: { id: 'asc' } });
  const specialNeeds = await prisma.specialNeed.findMany({ orderBy: { id: 'asc' } });
  const attachments = await prisma.attachment.findMany({ orderBy: { id: 'asc' } });
  const doctors = await prisma.doctor.findMany({ orderBy: { id: 'asc' } });
  const activities = await prisma.activityLog.findMany({ orderBy: { id: 'asc' } });
  ...
  - Special Needs: ${specialNeeds.length}
...
-- 5. DATA POPULATION (${students.length} students, ${checkups.length} checkups, ${observations.length} observations)
`;

  // Insert Students
  sql += `\n-- 5.1 Students\n`;
  for (const s of students) {
    sql += `INSERT INTO public."Student" ("admissionNumber", "studentName", "class", "section", "gender", "dob", "bloodGroup", "parentName", "phone", "createdAt", "updatedAt")\n` +
      `VALUES (${escapeSql(s.admissionNumber)}, ${escapeSql(s.studentName)}, ${escapeSql(s.class)}, ${escapeSql(s.section)}, ${escapeSql(s.gender)}, ${escapeSql(s.dob)}, ${escapeSql(s.bloodGroup)}, ${escapeSql(s.parentName)}, ${escapeSql(s.phone)}, ${escapeSql(s.createdAt)}, ${escapeSql(s.updatedAt)})\n` +
  ...
  for (const d of doctors) {
    sql += `INSERT INTO public."Doctor" ("id", "username", "password", "name", "role", "createdAt")\n` +
      `VALUES (${escapeSql(d.id)}, ${escapeSql(d.username)}, ${escapeSql(d.password)}, ${escapeSql(d.name)}, ${escapeSql(d.role)}, ${escapeSql(d.createdAt)})\n` +
  ...
  for (const c of checkups) {
    sql += `INSERT INTO public."HealthCheckup" ("id", "admissionNumber", "academicYear", "checkupDate", "height", "weight", "bmi", "eyesightLeft", "eyesightRight", "dentalHealth", "bloodPressure", "nutritionalStatus", "nutritionRemarks", "doctorName", "createdAt", "updatedAt")\n` +
      `VALUES (${escapeSql(c.id)}, ${escapeSql(c.admissionNumber)}, ${escapeSql(c.academicYear)}, ${escapeSql(c.checkupDate)}, ${escapeSql(c.height)}, ${escapeSql(c.weight)}, ${escapeSql(c.bmi)}, ${escapeSql(c.eyesightLeft)}, ${escapeSql(c.eyesightRight)}, ${escapeSql(c.dentalHealth)}, ${escapeSql(c.bloodPressure)}, ${escapeSql(c.nutritionalStatus)}, ${escapeSql(c.nutritionRemarks)}, ${escapeSql(c.doctorName)}, ${escapeSql(c.createdAt)}, ${escapeSql(c.updatedAt)})\n` +
  ...
  for (const o of observations) {
    sql += `INSERT INTO public."Observation" ("id", "admissionNumber", "academicYear", "observation", "recommendation", "createdAt", "updatedAt")\n` +
      `VALUES (${escapeSql(o.id)}, ${escapeSql(o.admissionNumber)}, ${escapeSql(o.academicYear)}, ${escapeSql(o.observation)}, ${escapeSql(o.recommendation)}, ${escapeSql(o.createdAt)}, ${escapeSql(o.updatedAt)})\n` +
  ...
  for (const i of immunizations) {
    sql += `INSERT INTO public."Immunization" ("id", "admissionNumber", "vaccine", "date", "dose", "nextDue", "remarks", "createdAt", "updatedAt")\n` +
      `VALUES (${escapeSql(i.id)}, ${escapeSql(i.admissionNumber)}, ${escapeSql(i.vaccine)}, ${escapeSql(i.date)}, ${escapeSql(i.dose)}, ${escapeSql(i.nextDue)}, ${escapeSql(i.remarks)}, ${escapeSql(i.createdAt)}, ${escapeSql(i.updatedAt)})\n` +
  ...
  for (const sn of specialNeeds) {
    const createdAtVal = sn.updatedAt || new Date();
    sql += `INSERT INTO public."SpecialNeed" ("id", "admissionNumber", "allergies", "chronicIllness", "disabilities", "learningDifficulties", "medication", "emergencyNotes", "createdAt", "updatedAt")\n` +
      `VALUES (${escapeSql(sn.id)}, ${escapeSql(sn.admissionNumber)}, ${escapeSql(sn.allergies)}, ${escapeSql(sn.chronicIllness)}, ${escapeSql(sn.disabilities)}, ${escapeSql(sn.learningDifficulties)}, ${escapeSql(sn.medication)}, ${escapeSql(sn.emergencyNotes)}, ${escapeSql(createdAtVal)}, ${escapeSql(sn.updatedAt)})\n` +
  ...
  for (const a of attachments) {
    sql += `INSERT INTO public."Attachment" ("id", "admissionNumber", "filename", "storedName", "fileUrl", "category", "uploadedDate")\n` +
      `VALUES (${escapeSql(a.id)}, ${escapeSql(a.admissionNumber)}, ${escapeSql(a.filename)}, ${escapeSql(a.storedName)}, ${escapeSql(a.fileUrl)}, ${escapeSql(a.category)}, ${escapeSql(a.uploadedDate)})\n` +
  ...
  for (const act of activities) {
    sql += `INSERT INTO public."ActivityLog" ("id", "actor", "role", "action", "details", "createdAt")\n` +
      `VALUES (${escapeSql(act.id)}, ${escapeSql(act.actor)}, ${escapeSql(act.role)}, ${escapeSql(act.action)}, ${escapeSql(act.details)}, ${escapeSql(act.createdAt)})\n` +
      `ON CONFLICT ("id") DO UPDATE SET "details" = EXCLUDED."details";\n`;
  }

  // 6. Reset Sequences to max id
  sql += `\n-- 6. RESET SEQUENCE COUNTERS\n`;
  sql += `SELECT setval(pg_get_serial_sequence('public."HealthCheckup"', 'id'), COALESCE(MAX(id), 1)) FROM public."HealthCheckup";\n`;
  sql += `SELECT setval(pg_get_serial_sequence('public."Observation"', 'id'), COALESCE(MAX(id), 1)) FROM public."Observation";\n`;
  sql += `SELECT setval(pg_get_serial_sequence('public."Immunization"', 'id'), COALESCE(MAX(id), 1)) FROM public."Immunization";\n`;
  sql += `SELECT setval(pg_get_serial_sequence('public."SpecialNeed"', 'id'), COALESCE(MAX(id), 1)) FROM public."SpecialNeed";\n`;
  sql += `SELECT setval(pg_get_serial_sequence('public."Attachment"', 'id'), COALESCE(MAX(id), 1)) FROM public."Attachment";\n`;
  sql += `SELECT setval(pg_get_serial_sequence('public."Doctor"', 'id'), COALESCE(MAX(id), 1)) FROM public."Doctor";\n`;
  sql += `SELECT setval(pg_get_serial_sequence('public."ActivityLog"', 'id'), COALESCE(MAX(id), 1)) FROM public."ActivityLog";\n`;

  const supabaseDir = path.join(process.cwd(), 'supabase');
  if (!fs.existsSync(supabaseDir)) fs.mkdirSync(supabaseDir, { recursive: true });

  const targetFile = path.join(supabaseDir, 'complete_supabase_setup.sql');
  fs.writeFileSync(targetFile, sql, 'utf8');

  // Split into schema and data
  const splitPoint = sql.indexOf('-- 5. DATA POPULATION');
  const schemaSql = sql.substring(0, splitPoint);
  const dataSql = sql.substring(splitPoint);

  fs.writeFileSync(path.join(supabaseDir, '01_schema.sql'), schemaSql, 'utf8');
  fs.writeFileSync(path.join(supabaseDir, '02_data.sql'), dataSql, 'utf8');

  const migrationFile = path.join(supabaseDir, 'migrations', '20260914000000_init_school_health_records.sql');
  fs.writeFileSync(migrationFile, sql, 'utf8');

  console.log(`\nSuccessfully regenerated verified Supabase migration scripts:`);
  console.log(`-> ${targetFile} (${(Buffer.byteLength(sql, 'utf8') / 1024).toFixed(1)} KB)`);
  console.log(`-> supabase/01_schema.sql (${(Buffer.byteLength(schemaSql, 'utf8') / 1024).toFixed(1)} KB)`);
  console.log(`-> supabase/02_data.sql (${(Buffer.byteLength(dataSql, 'utf8') / 1024).toFixed(1)} KB)`);
}
